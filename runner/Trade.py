...
class Trade(object):
    """ 0: timestamp, 1: buy_price, 2: size, 3: sell_price, 4. buy_order_id, 5: sell_order_id,
        6: 0-open buy, 2-fill buy, 9-open sell
    """
    ...
    def append(self, line_list):
        """need to change order_id to int for numpy, okex is 16bit char with digital
        """
    ...
    def get_open_buy_order_update_filled(self, spot):
        """(self.trade.info[: 6] == 0) - open buys orders, 剩下的状态是0的，置2
            TODO : first load have historical human placed orders, {1,2,3} - {1, 5}, ignore first
        """
        ...
        if r is not None:
            ...
            trade_open_buy_order_idx = set(np.argwhere(self.trade.info[: self.state_bit] == 0).ravel())
            rest_idx = list(trade_open_buy_order_idx - open_buy_order_idx)
            if len(rest_idx) > 0:
                self.trade.info[rest_idx, self.state_bit] = 2
            return r

    def have_around_open_buy_orders(self, low, high):
        condition = self.trade.info[:, self.state_bit] == 0
